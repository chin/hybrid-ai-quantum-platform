...
def test_returns_known_optimum_and_python_scalars() -> None:
    ...
    assert all(type(value) is int for value in candidate.values["sample"].values())
    assert candidate.native_metrics["is_reference"] is True
    assert candidate.resource_cost is not None
