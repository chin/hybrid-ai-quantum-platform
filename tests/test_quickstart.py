...
def test_quickstart_runs_exact_and_local_annealing(
    ...
    assert {evaluation.strategy for evaluation in recommendation.evaluations} == {
        "maxcut-exact",
        "maxcut-local-annealing",
    }
    assert recommendation.utility_assessment is not None
    assert recommendation.utility_assessment.selected_strategy in {
        evaluation.strategy for evaluation in recommendation.evaluations
    }
